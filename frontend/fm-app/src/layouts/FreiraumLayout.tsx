...
export default function FreiraumLayout({ robot, composer, ptt, exchange }: Props) {
  return (
    <div className="h-full w-full bg-[#07090d] text-white p-6">
      <div className="h-full w-full grid grid-cols-[minmax(360px,36%)_1fr] gap-6">
        ...
        {/* RIGHT: WORKBENCH */}
        <div className="min-h-0 h-full overflow-hidden grid grid-rows-[minmax(0,3fr)_minmax(0,2fr)] gap-5">
          {/* EXCHANGE MAIN AREA */}
          <div className="min-h-0 rounded-[34px] bg-[radial-gradient(circle_at_88%_10%,rgba(255,255,255,0.12),transparent_30%),linear-gradient(180deg,rgba(20,23,29,0.97),rgba(11,13,17,0.95))] shadow-[0_30px_55px_rgba(0,0,0,0.5)] p-6 flex flex-col">
            ...
            {exchange ? (
              <div className="min-h-0 h-full flex-1 overflow-hidden flex flex-col">
                {exchange}
              </div>
            ) : (
          ...
          {/* COMPOSER PREVIEW */}
          <div className="min-h-0 overflow-hidden rounded-[34px] bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.015))] shadow-[0_16px_34px_rgba(0,0,0,0.38)] p-4 flex items-start justify-center">
            <div className="w-full min-h-0 h-full flex items-start justify-center pt-1">
              {composer}
            </div>
          </div>
        </div>

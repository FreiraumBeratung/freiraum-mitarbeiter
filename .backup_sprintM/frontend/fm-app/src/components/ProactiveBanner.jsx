...
export default function ProactiveBanner({ onShowInsights }){
  ...
  return (
    <div className="fixed top-2 left-1/2 -translate-x-1/2 z-40 px-4 py-2 rounded-xl border border-fr-border bg-fr-panel/90 backdrop-blur text-fr-text shadow-fr-soft">
      <span className="font-medium">Freiraum Mitarbeiter</span> hat <span className="font-semibold text-fr-orange">{count}</span> Vorschläge. <button onClick={()=>{if(onShowInsights) onShowInsights('Insights');}} className="underline ml-2 text-fr-orange hover:text-fr-orange-dim">Anzeigen</button>
    </div>
  );
}
// === END FILE ===

    ...
    try:
        ...
        if recent >= 3:
            ...
            if not existing:
                sug = Suggestion(
                    kind="followup",
                    title=f"Kontakt aktiv: {email}",
                    detail="In den letzten Sessions mehrfach aktiv. Soll ich nachfassen?",
                    score=0.75,
                    data={"email": email},
...
def seed_suggestions(session: Session = Depends(get_session)):
    ...
    s1 = Suggestion(
        ...
        detail="Heute 3 Angebote offen, 2 Nachfassungen fällig.",
        score=0.6,
        data={},
        created_at=datetime.utcnow()
    )
    s2 = Suggestion(
        ...
        detail="3 Kontakte in 30 Tagen + 1 Angebot. Soll ich das prüfen?",
        score=0.7,

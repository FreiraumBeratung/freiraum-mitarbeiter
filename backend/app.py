import os, pathlib, socket, logging, sys, importlib.util
...
try:
    _health_ready = _load_router_module("health_ready")
    app.include_router(_health_ready.router, prefix="/api")  # type: ignore[attr-defined]
except Exception as e:
    log.warning(f"Health & Readiness router konnte nicht geladen werden: {e}")

# UI Router
try:
    _ui = _load_router_module("ui")
    app.include_router(_ui.router, prefix="/api")  # type: ignore[attr-defined]
except Exception as e:
    log.warning(f"UI router konnte nicht geladen werden: {e}")

# Lead Hunter Async Router - wird später mit anderen Routern geladen
...
@app.get("/api/ready")
def ready():
    # Ready wenn App hoch ist und mind. 1s läuft
    alive_for = (datetime.utcnow() - _start_time).total_seconds()
    return {"ok": True, "alive_for_s": alive_for}
...
try:
    ...
    try:
        from . import deep_voice as deep_voice_module
        deep_voice_router = getattr(deep_voice_module, "router", None)
    except Exception:
        deep_voice_router = None
try:
...
try:
    ...
    tts_router = None
# Lead Hunter Async Router - import after other routers
lead_async_router = None
try:
    from .routers.lead_async import router as lead_async_router
except Exception as e:
    try:
        import importlib.util
        router_path = os.path.join(BASE_DIR, "app", "routers", "lead_async.py")
        if os.path.exists(router_path):
            spec = importlib.util.spec_from_file_location("lead_async_router", router_path)
            lead_async_module = importlib.util.module_from_spec(spec)
            # Add backend root to path before executing
            backend_root = os.path.abspath(BASE_DIR.parent)
            if backend_root not in sys.path:
                sys.path.insert(0, backend_root)
            spec.loader.exec_module(lead_async_module)
            lead_async_router = lead_async_module.router
            log.info("Lead Async router geladen (direkt)")
    except Exception as e2:
        log.warning(f"Lead Async router konnte nicht geladen werden: {e}, {e2}")
...
# Lead Hunter Async Router
if lead_async_router:
    app.include_router(lead_async_router, prefix="/api")
    log.info("Lead Async router registriert")
if kb_module_router:
...
# DEEP VOICE ROUTER
if deep_voice_router:
    app.include_router(deep_voice_router)

# PROACTIVE ROUTER
try:
    proactive_router = _load_router_module("proactive")
    app.include_router(proactive_router.router, prefix="/api")  # type: ignore[attr-defined]
except Exception as e:
    log.warning(f"Proactive router konnte nicht geladen werden: {e}")
